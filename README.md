...
SDN allows centralized observation and control of traffic through the controller.
Without the controller, forwarding behavior does not function correctly in this setup.
The custom POX controller successfully classifies traffic into high, medium, and low priority categories.
ICMP traffic is small and delay-sensitive, making it suitable for high-priority observation.
HTTP traffic shows normal request-response behavior and represents moderate application-layer traffic.
iPerf traffic produces bulk transfer and occupies more bandwidth.
When all three traffic types are generated together, contention occurs at the bottleneck switch.
Wireshark confirms simultaneous packet traversal at the bottleneck.
The controller logs clearly distinguish traffic based on protocol and port.
17. Conclusion
